...
module.exports = async (less_files_path, options = {}) => {
  ...
  const files = await glob.sync(less_files_path);
  ...
  if (HASH_PATH) {
    ...
    if (fs.existsSync(hash_dir)) {
      if (fs.existsSync(hash_dir + HASH_NAME)) {
        const old_hashes = await JSON.parse(fs.readFileSync(hash_dir + HASH_NAME, 'utf8'));
  ...
  const imported_files = await invertFilesTree(dependencies_tree);
  const files_to_compile = await getFilesToCompile(changed_files, imported_files);
