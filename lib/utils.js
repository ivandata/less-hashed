 ...
 * @returns {Object}
 */
function getFilesImportedInByFilesImports(files_imports) {
    let imported_files = {};
 ...
 * @returns {Object}
 */
function getFilesWithDependencies(files, imported_file) {
    let compiled_files = {};
    ...
    for (let changed_file in files) {
        ...
        if (Object.keys(imported_file[changed_file]).length > 0) {
            let dependencies = getFilesWithDependencies(imported_file[changed_file], imported_file);
