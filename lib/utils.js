 ...
 * @param {String || boolean} base - base path
 * @returns {Array}
 */
function correctRelativePaths(file, imports, base) {
  const file_path = file.substring(0, file.lastIndexOf("/")) + '/';
  const zero_segment = file.startsWith('.') ? './' : '';

  imports.forEach((import_string, index) => {
    if (path.isAbsolute(import_string)) {
      if (!base) return;
      imports[index] = zero_segment + path.join(base, import_string);
      return;
    }
    imports[index] = zero_segment + path.join(file_path, import_string);
  });
...
/**
 * Get hash of file by using mtime
 * @param {string} file — path to file {"file"}
 * @returns {string}
 */
function getFileHash(file) {
  const stats = fs.statSync(file);
  const mtime = (new Date(stats.mtime) / 1000).toString();
  const hash = crypto.createHash('md5');

  return hash.update(mtime).digest('hex');
}
export {
  getFileHash,
  compareHash,
